...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected syk and zsyk supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zsyksupply']:
                assert_equal(node['zsyksupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check syk and zsyk supply at the beginning
        # ------------------------------------------
        # zsyk supply: 2 coins for each denomination
        ...
        # syk supply: block rewards minus burned fees for minting
        ...
        # Verify that syk and zsyk supplies were properly updated after the spends and reorgs
        self.log.info("Check syk and zsyk supply...")
