...
CzSYKWallet::CzSYKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsyk seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSYKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSYKSeed_deprecated()) {
                    LogPrintf("%s: Updated zSYK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsyk seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsyk wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSYKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSYKCount(nCountLastUsed);
    else if (!walletdb.ReadZSYKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSYKWallet::Lock()
{
    seedMaster = 0;
}

void CzSYKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSYKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSYK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSYKWallet::LoadMintPoolFromDB()
{
...
void CzSYKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSYKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSYKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsykTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSYKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSYK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsykTracker which also adds to database
    pwalletMain->zsykTracker->Add(dMint, true);
...
void CzSYKWallet::SeedToZSYK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSYKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSYKWallet::GenerateDeterministicZSYK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSYKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSYKWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSYKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
