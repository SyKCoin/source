...
CzSYKTracker::CzSYKTracker(std::string strWalletFile)
{
...
CzSYKTracker::~CzSYKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSYKTracker::Init()
{
...
bool CzSYKTracker::Archive(CMintMeta& meta)
{
...
bool CzSYKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSYKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSYKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSYKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSYKTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSYKTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSYKTracker::GetSerialHashes()
{
...
CAmount CzSYKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSYKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSYKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSYKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSYKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSYKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSYKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSYKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSYKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSYKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSYKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSYKWallet* zSYKWallet)
{
    bool iszSYKWalletInitialized = (NULL != zSYKWallet);
    CMintMeta meta;
    ...
    if (! iszSYKWalletInitialized)
        zSYKWallet = new CzSYKWallet(strWalletFile);
    meta.isSeedCorrect = zSYKWallet->CheckSeed(dMint);
    if (! iszSYKWalletInitialized)
        delete zSYKWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSYKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSYKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSYKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSYKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSYKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSYKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSYKWallet* zSYKWallet = new CzSYKWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSYKWallet);
        }
        delete zSYKWallet;
        LogPrint("zero", "%s: added %d dzsyk from DB\n", __func__, listDeterministicDB.size());
    }
