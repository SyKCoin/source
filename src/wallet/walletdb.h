...
public:
    ...
    bool WriteZSYKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSYKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSYKSeed_deprecated(uint256& seed);
    bool EraseZSYKSeed();
    bool EraseZSYKSeed_deprecated();

    bool WriteZSYKCount(const uint32_t& nCount);
    bool ReadZSYKCount(uint32_t& nCount);
