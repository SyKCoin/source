...
// The zSYK block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSYK is a hash of the serial
...
{
    //Create an output returning the zSYK that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSYKOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSYK output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSYK", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSYKOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSYK output", __func__);
...
{
    CzSYKTracker* zsykTracker = pwallet->zsykTracker.get();
    CMintMeta meta;
    if (!zsykTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zsykTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SYK stake is the outpoint
