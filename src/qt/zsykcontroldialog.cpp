...
#include "ui_zsykcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zsykTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsykTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSYK.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSYK seed used to mint this zSYK is not the same as currently hold in the wallet";
            else
