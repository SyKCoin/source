...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZSYKModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zsykTracker->HasMintTx(hash))) {
            //zSYK stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZSYK;
...
bool IsZSYKType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSYK:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZSYK || type == TransactionRecord::MNReward) {
